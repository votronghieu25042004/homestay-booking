          ...
          <div v-else class="space-y-4 max-w-3xl font-['Inter']">
            <div v-for="booking in bookings" :key="booking.id"
              ...
              <div v-if="booking.status !== 'cancelled'" class="px-5 pb-4 flex items-center gap-4 flex-wrap">
                ...
                <template v-if="booking.status === 'checked_out'">
                  <button
                    v-if="!booking.is_reviewed"
                  ...
                  <span
                    v-else
                    class="flex items-center gap-1.5 bg-gray-50 border border-gray-300 text-gray-500 font-bold text-sm px-4 py-1.5 rounded-xl"
                  >
                    ✅ Đã đánh giá
                  </span>
                </template>
...
const submitReview = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      reviewTarget.value.is_reviewed = true;
    } else {
